...
- (void)configUI
{
    if (self.authURLs.count > 0) {
        ...
        if (!_beginButton) {
            _beginButton = [UIButton buttonWithStyle:StrapSuccessStyle andTitle:@"开始验证" andFrame:CGRectMake(kPaddingLeftWidth, CGRectGetHeight(self.view.frame)- 20 - 45, SCREEN_WIDTH - kPaddingLeftWidth*2, 45) target:self action:@selector(beginButtonClicked:)];
            [self.view addSubview:_beginButton];
            [_beginButton mas_makeConstraints:^(MASConstraintMaker *make) {
                make.size.mas_equalTo(CGSizeMake(SCREEN_WIDTH - kPaddingLeftWidth * 2, 45));
                make.centerX.equalTo(self.view);
                make.bottom.equalTo(self.view).offset(-20);
            }];
        }
        CGSize tipImageSize = tipImage.size;
        CGFloat imageScale = 0.75, labelScale = 0.75;
...
{
    NSArray *otpAccountListArray = [SAMKeychain accountsForService:kOTPKeychainEntriesArray];
    NSMutableArray *authURLs = [NSMutableArray arrayWithCapacity:otpAccountListArray.count];
    for (NSDictionary *otpAccount in otpAccountListArray) {
