...
- (void)tabBarItemClicked
{
    ...
    } else if (!_refreshControl.isAnimating) {
        [_refreshControl beginRefreshing];
        [_myTableView setContentOffset:CGPointMake(0, -44)];
        [self refresh];
    }
}
...
- (void)removeConversation:(PrivateMessage *)curMsg inTableView:(UITableView *)tableView{
    kLog(@"removeConversationWithFriend : %@", curMsg.friend.name);
    __weak typeof(self) weakSelf = self;
