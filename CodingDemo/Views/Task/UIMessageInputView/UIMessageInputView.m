...
- (void)setFrame:(CGRect)frame{
    CGFloat oldheightToBottom = SCREENH_HEIGHT - CGRectGetMinY(self.frame);
    CGFloat newheightToBottom = SCREENH_HEIGHT - CGRectGetMinY(frame);
    [super setFrame:frame];
    if (fabs(oldheightToBottom - newheightToBottom) > 0.1) {
        kLog(@"heightToBottom-----:%.2f", newheightToBottom);
...
- (void)prepareToShow{
    ...
    if (_isAlwaysShow && ![self isCustomFirstResponder]) {
        [UIView animateWithDuration:0.25 animations:^{
            [self setY:SCREENH_HEIGHT - CGRectGetHeight(self.frame)];
        }];
    }
}
...
- (BOOL)isAndResignFirstResponder{
    if (self.inputState == UIMessageInputViewStateAdd || self.inputState == UIMessageInputViewStateEmotion || self.inputState == UIMessageInputViewStateVoice) {
        [UIView animateWithDuration:0.25 delay:0.0f options:UIViewAnimationOptionTransitionFlipFromBottom animations:^{
            [_emojiKeyboardView setY:SCREENH_HEIGHT];
            [_addKeyboardView setY:SCREENH_HEIGHT];
            [_voiceKeyboardView setY:SCREENH_HEIGHT];
...
+ (instancetype)messageInputViewWithType:(UIMessageInputViewContentType)type placeHolder:(NSString *)placeHolder{
    UIMessageInputView *messageInputView = [[UIMessageInputView alloc] initWithFrame:CGRectMake(0, SCREENH_HEIGHT, SCREEN_WIDTH, kMessageInputView_Height)];
...
- (void)customUIWithType:(UIMessageInputViewContentType)type{
    ...
    if (!_inputTextView) {
        _inputTextView = [[UIPlaceHolderTextView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH - kPaddingLeftWidth - toolBtnNum *kMessageInputView_Width_Tool - (hasVoiceBtn ? 7+kMessageInputView_Width_Tool+7 : kPaddingLeftWidth), contentViewHeight)];
    ...
    if (hasEmotionBtn && !_emotionButton) {
        _emotionButton = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - kPaddingLeftWidth/2 - toolBtnNum * kMessageInputView_Width_Tool, (kMessageInputView_Height - kMessageInputView_Width_Tool)/2, kMessageInputView_Width_Tool, kMessageInputView_Width_Tool)];
    ...
    if (hasAddBtn && !_addButton) {
        _addButton = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - kPaddingLeftWidth/2 -kMessageInputView_Width_Tool, (kMessageInputView_Height - kMessageInputView_Width_Tool)/2, kMessageInputView_Width_Tool, kMessageInputView_Width_Tool)];
    ...
    if (hasPhotoBtn && !_photoButton) {
        _photoButton = [[UIButton alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - kPaddingLeftWidth/2 -kMessageInputView_Width_Tool, (kMessageInputView_Height - kMessageInputView_Width_Tool)/2, kMessageInputView_Width_Tool, kMessageInputView_Width_Tool)];
    ...
    if (hasEmotionBtn && !_emojiKeyboardView) {
        _emojiKeyboardView = [[AGEmojiKeyboardView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, kKeyboardView_Height) dataSource:self showBigEmotion:showBigEmotion];
        _emojiKeyboardView.delegate = self;
        [_emojiKeyboardView setY:SCREENH_HEIGHT];
    }
    
    if (hasAddBtn && !_addKeyboardView) {
        _addKeyboardView = [[UIMessageInputView_Add alloc] initWithFrame:CGRectMake(0, SCREENH_HEIGHT, SCREEN_WIDTH, kKeyboardView_Height)];
    ...
    if (hasVoiceBtn && !_voiceKeyboardView) {
        _voiceKeyboardView = [[UIMessageInputView_Voice alloc] initWithFrame:CGRectMake(0, SCREENH_HEIGHT, SCREEN_WIDTH, kKeyboardView_Height)];
...
- (void)updateContentViewBecauseOfMedia:(BOOL)becauseOfMedia{
    ...
    CGFloat maxSelfHeight = SCREENH_HEIGHT/2;
    if (kDevice_Is_iPhone5){
        maxSelfHeight = 230;
    }else if (kDevice_Is_iPhone6) {
        maxSelfHeight = 290;
    }else if (kDevice_Is_iPhone6Plus){
        maxSelfHeight = SCREENH_HEIGHT/2;
    }else{
        maxSelfHeight = 140;
    }
...
- (void)addButtonClicked:(id)sender{
    ...
    if (self.inputState == UIMessageInputViewStateAdd) {
        ...
        [_inputTextView resignFirstResponder];
        endY = SCREENH_HEIGHT - kKeyboardView_Height;
    }
    [UIView animateWithDuration:0.25 delay:0.0f options:UIViewAnimationOptionTransitionFlipFromBottom animations:^{
        [_addKeyboardView setY:endY];
        [_emojiKeyboardView setY:SCREENH_HEIGHT];
        [_voiceKeyboardView setY:SCREENH_HEIGHT];
        if (ABS(SCREENH_HEIGHT - endY) > 0.1) {
        ...
        [RACObserve(uploadTask, progress.fractionCompleted) subscribeNext:^(NSNumber *fractionCompleted) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if (self.HUD) {
                    ...
                    kLog(@"uploadingPhotoName - %@ : %.2f", self.uploadingPhotoName, fractionCompleted.floatValue);
                }
            });
        }];
    }else{
...
- (void)keyboardChange:(NSNotification*)aNotification{
    ...
    if (self.inputState == UIMessageInputViewStateSystem && [self.inputTextView isFirstResponder]) {
        ...
        CGFloat selfOriginY = keyboardY == SCREENH_HEIGHT? self.isAlwaysShow? SCREENH_HEIGHT - CGRectGetHeight(self.frame): SCREENH_HEIGHT : keyboardY - CGRectGetHeight(self.frame);
//        if (keyboardY == SCREENH_HEIGHT) {
//            if (self.isAlwaysShow) {
//                selfOriginY = SCREENH_HEIGHT- CGRectGetHeight(self.frame);
//            }else{
//                selfOriginY = SCREENH_HEIGHT;
//            }
//        }else{
