...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        if (!_time) {
            _time = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 100-kPaddingLeftWidth, 8, 100, 25)];
        ...
        if (!_msg) {
            _msg = [[UILabel alloc] initWithFrame:CGRectMake(75, 30, SCREEN_WIDTH-75-30 -kPaddingLeftWidth, 25)];
...
- (void)layoutSubviews{
    ...
    [self.contentView addBadgeTip:badgeTip withCenterPosition:CGPointMake(SCREEN_WIDTH-25, CGRectGetMaxY(_time.frame) +10)];
}
