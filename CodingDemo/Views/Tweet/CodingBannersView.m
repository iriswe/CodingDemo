...
- (instancetype)init
{
    ...
    if (self) {
        _padding_top = 40;
        _padding_bottom = 15;
        _image_width = SCREEN_WIDTH - 2*kPaddingLeftWidth;
        _ratio = 0.3;
        CGFloat viewHeight = _padding_top + _padding_bottom + _image_width * _ratio;
        [self setSize:CGSizeMake(SCREEN_WIDTH, viewHeight)];
    }
    return self;
}
...
- (void)setCurBannerList:(NSArray *)curBannerList
{
    ...
    if (!_myPageControl) {
        _myPageControl = ({
            ...
            pageControl.frame = (CGRect){CGRectGetMaxX(_titleLabel.frame) + 5, (_padding_top - 10)/2, SCREEN_WIDTH - CGRectGetMaxX(_titleLabel.frame) - kPaddingLeftWidth - 5, 10};
