...
#define kTweetCell_ContentWidth (SCREEN_WIDTH -kTweetCell_PadingLeft - kPaddingLeftWidth)
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        if (!_topView) {
            _topView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 15)];
        ...
        if (!self.timeLabel) {
            self.timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMinX(self.ownerNameBtn.frame), 0, SCREEN_WIDTH/2, 12)];
        ...
        if (!self.commentBtn) {
            CGRect frame = CGRectMake(SCREEN_WIDTH - kPaddingLeftWidth- kTweetCell_LikeComment_Width, 0, kTweetCell_LikeComment_Width, kTweetCell_LikeComment_Height);
        ...
        if (!self.deleteBtn) {
            ...
            self.deleteBtn.frame = CGRectMake(SCREEN_WIDTH - kPaddingLeftWidth- 2*kTweetCell_LikeComment_Width -5, 0, kTweetCell_LikeComment_Width, kTweetCell_LikeComment_Height);
        ...
        if (!self.locaitonBtn) {
            ...
            self.locaitonBtn.frame = CGRectMake(kTweetCell_PadingLeft, 0, SCREEN_WIDTH - kTweetCell_PadingLeft - kPaddingLeftWidth, 15);
        ...
        if (!self.fromLabel) {
            self.fromLabel = [[UILabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.fromPhoneIconView.frame) + 5, 0, SCREEN_WIDTH/2, 15)];
...
- (void)tableView:(UITableView *)tableView performAction:(SEL)action forRowAtIndexPath:(NSIndexPath *)indexPath withSender:(id)sender {
    if (action == @selector(copy:)) {
        Comment *curCom = [_tweet.comment_list objectAtIndex:indexPath.row];
        [UIPasteboard generalPasteboard].string = curCom.content ? curCom.content: @"";
    }
}
